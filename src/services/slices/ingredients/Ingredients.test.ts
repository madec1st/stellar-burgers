...
const movedUpIngredientState = ingredientReducer(
  mocks.selectedFillingMock,
  actionMoveUp
);
const movedDownIngredientState = ingredientReducer(
  mocks.selectedFillingMock,
  actionMoveDown
);
const stateWithRemovedIngredient = ingredientReducer(
  mocks.selectedFillingMock,
  actionRemoveFilling
);
...
describe('tests for move ingredients', () => {
  test('moveIngredientUp test', () => {
    ...
    test('moveIngredientDown test', () => {
      expect(movedDownIngredientState).toEqual(
        mocks.expectedStateWithMovedDown
      );
    });
});
...
describe('test for remove ingredient', () => {
  test('removeFilling test', () => {
    expect(stateWithRemovedIngredient).toEqual(
      mocks.expectedStateWithRemovedIngredient
    );
  });
});
